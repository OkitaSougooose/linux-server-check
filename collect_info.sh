...
# 2. CPU信息+实时使用率采集
collect_cpu_info() {
    echo "[INFO] $(date '+%Y-%m-%d %H:%M:%S') 开始采集CPU信息"
    echo "==================== 【 CPU 信 息 】 ===================="
    ...
    if [ -z "$cpu_model" ]; then
        cpu_model=$(grep -m1 'Processor' /proc/cpuinfo | cut -d: -f2 | xargs 2>/dev/null)
    fi
    [ -z "$cpu_model" ] && cpu_model="未知"

    # 3. CPU主频（MHz）
    local cpu_mhz=$(grep -m1 'cpu MHz' /proc/cpuinfo | cut -d: -f2 | xargs 2>/dev/null)
    if [ -z "$cpu_mhz" ] || [ "$cpu_mhz" = "" ]; then
        # 树莓派专用：从系统时钟获取
        cpu_mhz=$(cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq 2>/dev/null)
        if [ -n "$cpu_mhz" ]; then
            cpu_mhz=$(echo "scale=0; $cpu_mhz / 1000" | bc)
        else
            cpu_mhz="未知"
        fi
    fi

    # 4. 实时CPU使用率（两次/proc/stat差值计算）
    local cpu_usage
    read -r user1 nice1 sys1 idle1 iowait1 irq1 softirq1 steal1 < <(awk '/^cpu / {print $2, $3, $4, $5, $6, $7, $8, $9}' /proc/stat)
    local total1=$((user1 + nice1 + sys1 + idle1 + iowait1 + irq1 + softirq1 + steal1))
    local idle_total1=$((idle1 + iowait1))

    sleep 0.2

    read -r user2 nice2 sys2 idle2 iowait2 irq2 softirq2 steal2 < <(awk '/^cpu / {print $2, $3, $4, $5, $6, $7, $8, $9}' /proc/stat)
    ...
    if [ $total_diff -gt 0 ]; then
        cpu_usage=$(echo "scale=2; 100 - 100 * $idle_diff / $total_diff" | bc 2>/dev/null)
        if (( $(echo "$cpu_usage < 0" | bc -l 2>/dev/null) )); then
            cpu_usage="0.00"
        elif (( $(echo "$cpu_usage > 100" | bc -l 2>/dev/null) )); then
            cpu_usage="100.00"
        fi
    else
        cpu_usage="未知"
    ...
    echo "CPU 实时使用率：${cpu_usage}%（阈值：${CPU_USAGE_THRESHOLD:-80}%）"

    # 告警提示
    if [ "$cpu_usage" != "未知" ] && (( $(echo "$cpu_usage > ${CPU_USAGE_THRESHOLD:-80}" | bc -l 2>/dev/null) )); then
        echo -e "${YELLOW}[WARN] CPU使用率超过阈值！当前：${cpu_usage}%，阈值：${CPU_USAGE_THRESHOLD:-80}%${NC}"
    fi
